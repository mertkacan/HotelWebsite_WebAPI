...
namespace Hotel.WebUI.Controllers
{
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMesaj = await client.GetAsync("http://localhost:5212/api/Staff"); // Hangi url'e istekte bulunacağımı belirttim ve listeleme için GetAsync kullandım. 
            if (responseMesaj.IsSuccessStatusCode)// Eğer başarılı durum kodu dönerse
            {
                ...
                var values = JsonConvert.DeserializeObject<List<StaffViewModel>>(jsonData);// Gelen veriyi Deserialize et dedim.
        ...
        public async Task<IActionResult> AddStaff(AddStaffViewModel addStaffViewModel)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5212/api/Staff", stringContent); // Ekleme yapıcağım için PostAsync kullandım.
        ...
        {
            var client = _httpClientFactory.CreateClient();// Bir tane istemci oluşturdum.
            var responseMessage = await client.DeleteAsync($"http://localhost:5212/api/Staff/{id}"); // Silme yapıcağım için DeleteAsync kullandım. $ sembolü ile ilgili id parametresini URL.2nin yanında belirttim.
        ...
        public async Task<IActionResult> UpdateStaff(int id) 
        {
            ...
            var responseMessage = await client.GetAsync($"http://localhost:5212/api/Staff/{id}"); // Güncellemek istediğim verileri GetAsync ile getirdim.
            if (responseMessage.IsSuccessStatusCode)//Başarılı dönerse
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdateStaffViewModel>(jsonData);// jsonData ile karşılanan veriyi Deserialize yaptık.
                return View(values);
            }
            return View();
        }
